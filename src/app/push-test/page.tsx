...
// account (unlike Safari, which is a separate session on iOS), reports your saved
// subscriptions, and sends a test push right now. Remove once push is confirmed.
export default async function PushTest() {
  ...
  if (!user) return <main style={box}><h2>Push test</h2><p>Not signed in here. Open this from inside the app while logged in.</p></main>;
  ...
  return (
    <main style={box}>
      ...
      <p style={{ marginTop: 16, color: '#555' }}>
        {sent > 0
          ? 'A notification should appear now. If it does, push works — tell me and I’ll remove this page.'
          : (subs?.length ?? 0) === 0
            ? 'No devices saved for this account yet. Make sure notifications are enabled in this app.'
